...
type Database struct {
	...
	Port        int
	Charset     string
	// SSL defines the encryption requirement when connecting to SQL Server. Possible values:
	// "disable"  – never use TLS; "prefer" (default) – attempt TLS but fall back to plain;
	// "require"  – always enforce TLS. Only applicable to SQL Server back-end.
	SSL        string `ini:"ssl"`
	UnixSocket bool
	// DisableAutoMigration disables the automatic schema creation/migration step that
	// Cloudreve normally performs at start-up. This is useful when running against
	// database engines whose DDL cannot be generated by Ent (e.g. Microsoft SQL
	// Server) and you prefer to manage the schema manually.
	DisableAutoMigration bool `ini:"DisableAutoMigration"`
}
