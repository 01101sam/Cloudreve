...
func NewFileClient(client *ent.Client, dbType conf.DBType, hasher hashid.Encoder) FileClient {
	return &fileClient{client: client, maxSQlParam: sqlParamLimit(dbType), hasher: hasher, dbType: dbType}
}
...
func (c *fileClient) SetClient(newClient *ent.Client) TxOperator {
	return &fileClient{client: newClient, maxSQlParam: c.maxSQlParam, hasher: c.hasher, dbType: c.dbType}
}
...
func (f *fileClient) Root(ctx context.Context, user *ent.User) (*ent.File, error) {
	// The query generated by `QueryFiles` uses a JOIN that leads Ent to emit
	// `SELECT TOP 1 DISTINCT`, which is not valid T-SQL.  We therefore fall
	// back to a simpler query on SQL Server that doesn't require DISTINCT.
	if f.dbType == conf.MsSqlDB {
		return f.client.File.Query().
			Where(file.OwnerID(user.ID)).
			Where(file.FileChildrenIsNil()).
			Where(file.Name(RootFolderName)).
			// Avoid DISTINCT clause entirely.
			Unique(false).
			First(ctx)
	}
...
func (f *fileClient) CreateFolder(ctx context.Context, root *ent.File, args *CreateFolderParameters) (*ent.File, error) {
	...
	var newFolder *ent.File
	var err error

	// MSSQL does not support the "ON CONFLICT" clause generated by Ent. Fall back to a
	// simple INSERT and handle unique-constraint violations manually.
	if f.dbType == conf.MsSqlDB {
		newFolder, err = stm.Save(ctx)
		if err != nil {
			if ent.IsConstraintError(err) {
				// Folder already exists. Query and return the existing row.
				query := f.client.File.Query().
					Where(file.OwnerID(args.Owner), file.Name(args.Name))
				if root != nil {
					query = query.Where(file.HasParentWith(file.ID(root.ID)))
				} else {
					query = query.Where(file.Not(file.HasParent()))
				}

				newFolder, err = query.First(ctx)
				if err != nil {
					return nil, fmt.Errorf("failed to fetch existing folder: %w", err)
				}
			} else {
